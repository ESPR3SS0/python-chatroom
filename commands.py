...
class ClientCommandInfo(Enum):
    ...
    GROUPPOST = CommandInfo("grouppost", "usage: grouppost <group>; <subject>; <msg>", ['group', 'subject', 'msg'])

    GROUPUSERS   = CommandInfo("groupusers", "usage: groupusers <group>",['group'])
    GROUPLEAVE   = CommandInfo("groupleave", "usage: groupleave <group>",['group'])
...
def gen_client_cmd(command_name: str, **kwargs) -> Command:
    ...
    for cmd in [x.value for  x in ClientCommandInfo]:
        ...
        # Check for the correct number of args
        check_client_cmd_args(kwargs.keys(), cmd)
