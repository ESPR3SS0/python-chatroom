...
class SingleGroupServer:
    ...
    def session_loop(self, packet: Packet, client_conn: ClientConnection) -> list[QueueObject]:
        ...
        match cmd:
            ...
            case ClientCommands.JOIN | ClientCommands.GROUPJOIN:
...
class MasterGroupServer:
    ...
    def session(self):
        ...
        writables = list(x for x, q  in self.packets_for_conn.items() if len(q) > 0)
            ...
            match recv_packet.header:
                ...
                case PayloadTypes.CLIENT_COMMAND:
                    ...
                    if not isinstance(recv_packet.contents, Command):
                        # If it is not a string there's an issue
                        print("[ERROR] Payload type of info has non-Command contents")
                        self.create_and_queue_error(client_conn)
                        return
                    ...
                    try:
                        if cmd.name == str(ClientCommands.GROUPS.value):
                            ...
                            serial_payload = gen_serialized_packet(PayloadTypes.SERVER_INFO, group_str)

                            # Create the queue payload 
                            queue_payload = QueueObject(serial_payload, [client_conn])
                        ...
                        if "group" in cmd.name.lower():
                            ...
                            print(f"[sub_WRITE] Should be generating group sepecifc resp:")
                            print(f"[sub_WRITE] The command name is: {cmd.name}")
                            print(f"[sub_WRITE] The command {cmd}")

                            # parse the group that they want to send this to
                            group_name = cmd.args['group']
                            print(f"[sub_WRITE] The group_name is: {group_name}")
                            ...
                            print(f"[sub_WRITE] Got {len(queue_payloads)} payloads")
    ...
    def clear_client_conn(self, client_conn: ClientConnection) -> None:
        ...
        pseudo_leave_cmd = gen_client_cmd_from_raw(str(ClientCommands.LEAVE.value))
